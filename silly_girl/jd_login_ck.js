 ...
 * @title 京东ck登录
 * @origin David
 * @rule raw (^pt_key=\s*[^;]*;\s*pt_pin=\s*[^;]*;)|(^pt_pin=\s*[^;]*;\s*pt_key=\s*[^;]*;)|(^wskey=\s*[^;]*;\s*pin=\s*[^;]*;)|(^pin=\s*[^;]*;\s*wskey=\s*[^;]*;)
 * @author david
 * @version v0.0.1
 * @description 京东ck登录
...
async function main() {
  ...
  let pattern = /(^pt_key=\s*[^;]*;\s*pt_pin=\s*[^;]*;)|(^pt_pin=\s*[^;]*;\s*pt_key=\s*[^;]*;)/
  let patternWsky = /(^wskey=\s*[^;]*;\s*pin=\s*[^;]*;)|(^pin=\s*[^;]*;\s*wskey=\s*[^;]*;)/

  let type = ''

  if (pattern.test(msg)) {
    type = 'jd_ck'
  } else if (patternWsky.test(msg)) {
    type = 'jd_wsky'
  } else {
    s.reply(`格式不对，请检查格式是否为pt_key=xx;pt_pin=xx;或者pin=xxx;wskey=xxx;`)
    return
  }
  ...
  cks.forEach((item) => {
    ...
    if (_ck.pt_pin == pushJdCk.pt_pin || _ck.pin == pushJdCk.pin) {
      // 存在QL里面
      isExistCk = item
    }
  })
  ...
  if (isExistCk) {
    ...
    let res = client.createEnvs([{ name: type == 'jd_wsky' ? "JD_WSCK" : "JD_COOKIE", value: msg, remarks: type == 'jd_wsky' ? pushJdCk.pin : pushJdCk.pt_pin }])
